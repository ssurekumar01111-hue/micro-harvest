...
class GateBloc extends Bloc<GateEvent, GateState> {
  ...
  Future<void> _onLoadGateDetails(LoadGateDetails event, Emitter<GateState> emit) async {
    emit(GateLoading());
    try {
      final gps = await _locationRepository.getCurrentLocation();
      // Need a getHandoffById in HaulRepository but for now just mock
      // emit(GateLoaded(handoff: ..., currentGPS: gps));
    } catch (e) {
